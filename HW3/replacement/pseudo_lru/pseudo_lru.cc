...
pseudo_lru::pseudo_lru(CACHE *cache)
    : pseudo_lru(cache, cache->NUM_SET, cache->NUM_WAY) {}
...
long pseudo_lru::find_victim(uint32_t triggering_cpu, uint64_t instr_id,
  ...
  while (left != right) {
    ...
    if (tree_bits.at(static_cast<std::size_t>(set))
            .at(static_cast<std::size_t>(node)) == 0) {
      node = 2 * node + 1;
      right = mid;
    } else {
      node = 2 * node + 2;
...
void pseudo_lru::touch(long set, long way) {
  ...
  while (left != right) {
    ...
    if (way <= mid) {
      ...
      tree_bits.at(static_cast<std::size_t>(set))
          .at(static_cast<std::size_t>(node)) = 1;
      node = 2 * node + 1;
      right = mid;
    } else {
...
void pseudo_lru::replacement_cache_fill(uint32_t triggering_cpu, long set,
...
void pseudo_lru::update_replacement_state(uint32_t triggering_cpu, long set,
                                          long way, champsim::address full_addr,
                                          champsim::address ip,
                                          champsim::address victim_addr,
