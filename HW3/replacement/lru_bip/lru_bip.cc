...
lru_bip::lru_bip(CACHE *cache)
    : lru_bip(cache, cache->NUM_SET, cache->NUM_WAY) {}
...
void lru_bip::replacement_cache_fill(uint32_t triggering_cpu, long set,
...
void lru_bip::update_replacement_state(uint32_t triggering_cpu, long set,
                                       long way, champsim::address full_addr,
                                       champsim::address ip,
                                       champsim::address victim_addr,
  ...
  if (hit &&access_type{type} != access_type::WRITE)
    last_used_cycles.at(static_cast<std::size_t>(set * NUM_WAY + way)) =
        cycle++;
}
